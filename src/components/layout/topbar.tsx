...
/** جرس التنبيهات الآن حقيقي: عدّاد غير المقروء + الضغط يفتح /alerts مع قائمة جانبية للموبايل. */
      ...
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-line bg-card/80 px-4 backdrop-blur md:px-6">
        <div className="flex min-w-0 items-center gap-1">
          <button 
            ...
            className="rounded-el p-2 text-ink-soft hover:bg-paper lg:hidden"
          >
          ...
          <nav className="rise absolute inset-y-0 start-0 flex w-72 max-w-[85vw] flex-col bg-card shadow-pop">
            <div className="flex items-center justify-between border-b border-line px-4 py-4">
              <span className="flex items-center gap-2">
                <span className="grid size-8 place-items-center rounded-el bg-primary text-white">
              ...
              {items.map(({ href, label, icon: Icon }) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "flex items-center gap-3 rounded-el px-3 py-3 text-sm font-medium",
