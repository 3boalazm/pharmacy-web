...
import { importBaseCatalog, listMedicines } from "../api";
...
export function ProductsTable() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["medicines.list", search, sort, stockStatus],
    queryFn: ({ signal }) => listMedicines({ search: search || undefined, sort, stockStatus: stockStatus || undefined }, signal),
    select: (r) => r.data,
  });
  ...
  return (
    <Card>
      <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3">
        ...
        <select value={sort} onChange={(e) => setSort(e.target.value)} className="h-9 rounded-el border border-line bg-card px-2 text-sm">
        ...
        <div className="flex gap-1">
          {[{ k: "", l: "الكل" }, { k: "in", l: "متوفر" }, { k: "low", l: "منخفض" }, { k: "out", l: "نافد" }].map((f) => (
            <button key={f.k} onClick={() => setStockStatus(f.k)}
              className={`rounded-el px-2.5 py-1 text-xs font-bold ${stockStatus === f.k ? "bg-primary text-white" : "bg-paper text-ink-soft hover:bg-line"}`}>
              {f.l}
            </button>
          ))}
        </div>
